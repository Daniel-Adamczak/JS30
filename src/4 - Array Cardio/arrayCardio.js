...
// 8. Reduce Exercise
// Sum up the instances of each of
function countOccurences() {
  const stringValues = [...new Set([...data])];

  return stringValues.map((currentString) => {
    return {
      currentString: data.filter((givenString) => givenString === currentString)
        .length,
    };
  });
}
